...
export default {
  ...
  methods: {
      ...
      async stopAll(){
        ...
        await this.screen.getTracks().forEach(function(track) {
        track.stop();
        });
      }
  },
  ...
  async mounted() {
    ...
    displaymediastreamconstraints = { audio: true, video: { width: 1280, height: 720 } }
    if(navigator.mediaDevices.getDisplayMedia) {
        this.screen = await navigator.mediaDevices.getDisplayMedia(displaymediastreamconstraints);
    }
    else {
        this.screen = await navigator.getDisplayMedia(displaymediastreamconstraints)
    }

    this.recorder = RecordRTC([this.screen, this.stream], {
            type: 'video',
            mimeType: 'video/webm',
            previewStream: function(s) {
                // document.getElementById("videoRTC_").muted = true;
                // document.getElementById("videoRTC_").srcObject = s;
            }
        });
    document.getElementById("videoRTC_").srcObject = this.screen;
  },
};
</script>
