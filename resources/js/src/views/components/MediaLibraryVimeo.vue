    ...
    <vs-col vs-w="6">
      <div style="">
        ...
        <div style="height: calc(100vh - 231px);">
            <form v-on:submit.prevent="getVimeoLinks_" v-if="!links"  >
                <vs-input v-model="video_url" required label="Vimeo URL" placeholder="Vimeo URL"/>
            ...
            <template v-else>
                <div class="customForm"   >
                    ...
                    <option v-for="link,i in links " :value="link.url" :key="i">{{link.quality}}</option>
                    </select>
                </div>
...
export default {
  ...
  methods: {
    ...
    async getVimeoLinks_(){
        this.links = false
        
      this.$vs.loading({type:"sound"})
       let res = await this.getVimeoLinks(this.video_url)           
